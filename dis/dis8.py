...
def strange_loop():
    ...
    """
    "*** YOUR CODE HERE ***"
    s=Link(random.randint(1,100))
    s.rest=Link(Link(random.randint(1,100),s))
    return s
...
def sum_rec(s, k):
    ...
    "*** YOUR CODE HERE ***"
    if (isinstance(s,Link) and s.first): # s is not empty and suming its first value
        if(k==0):
            return 0
        else:
...
def sum_iter(s, k):
    ...
    "*** YOUR CODE HERE ***"
    sum=0
    while(k and isinstance(s,Link) and s.first):
        sum+=s.first
        k-=1
        s=s.rest
    return sum
def overlap(s, t):
    ...
    "*** YOUR CODE HERE ***"
    count=0
    while(isinstance(s,Link) and isinstance(t,Link) and s.first!=s.empty and t.first!=t.empty):
        if(s.first==t.first):
            count+=1
            s=s.rest
            t=t.rest

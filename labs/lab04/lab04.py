...
def prune_leaves(t, vals):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Modify the primitive tree directly leading our traversal's index error.
    """if is_leaf(t):
        if label(t) in vals:            
            return None
        else:
        ...
        new_branche=[]
        for x in branches(t):
            new_branch=prune_leaves(x,vals)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    default=make_city('dasdsada',lat, lon)
    if distance(city_a,default) > distance(city_b,default):
...
def buy(fruits_to_buy, prices, total_amount):
    ...
    [9 apples][1 kiwi]
    """
    # new_cart aiming to protect cart from modifying
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(amount//price):
                ...
                new_cart=cart+display(fruit, k+1)
                add(fruits[1:], amount-price*(k+1), new_cart)
...
def count_palindromes(L):
    ...
    2
    """
    return len([x for x in L if x[::-1].lower() == x.lower()])

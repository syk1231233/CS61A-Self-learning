...
def calc_eval(exp):
    ...
    3
    """
    """ 1. find the operator and translate it into python's operator
        2. calc_eavl the operand and return its final evaluation
        3. apply the operator on the final operand
    """
    if isinstance(exp, Pair):
        ...
        # what above are not the arithmetic operator
...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    # in the function calc_eval, when the exp is "//" , then it will call this funcition on the opreand
    """ 1. divide the operand into two part
        2. use the 
    """
    if(args.rest is nil):
        return args.first
...
def eval_and(expressions):
    ...
    True
    """
    """ 1. if the expressions is nil,return True 
        the expression is a type of pair so we must modify it through to it only consist in a calc_value and nil
        2. if the calc_eval is False ,stop! and return false 
           else abandon the expression.first, return expressions.rest
        finally,we get a Pair whose structure is a value and nil
        3. return the value
    """
    if(expressions is nil):
        return True
    elif(isinstance(expressions,Pair)):
...
def eval_define(expressions):
    ...
    2
    """
    """  1. translate the expressions' defined str into value
         2. if it defined, return its binding
            if not, binding it with expressions.first then return the defined str
            in the process, if expresssions.rest.first is in OPERATORS ,we must eval it first
                                                        if not ,bind it expressions.rest.first directly
    """
    symbol, value = expressions.first, calc_eval(expressions.rest.first)
    bindings[symbol] = value
    return symbol
    """travel=expressions.rest
    while(travel is not nil):
        if(travel.first in bindings):
            travel.first=bindings[travel.first]
        travel=travel.rest
    if(expressions.first in bindings):
        return bindings[expressions.first]
    else:
        if(expressions.rest.first in OPERATORS):

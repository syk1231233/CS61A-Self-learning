...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    def filter(k):
        dir=split(remove_punctuation(k))  
        for x in dir:
            if lower(x) in keywords:
                return True
        return False
    return filter
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    for index in range(min(len(typed_words),len(source_words))):
        if typed_words[index]== source_words[index] or typed_words[index]=='\t'+source_words[index] or typed_words[index]== source_words[index]+'\t':
            nominator+=1
    denominator=len(typed_words)
    if denominator==0 and len(source_words)!=0:
        return 0.0
    elif denominator!=0 and nominator==0:
        return 0.0
    elif denominator==0 and len(source_words)==0:
        return 100.0
    else:
...
def memo_diff(diff_function):
    ...
    When memoized sees a (typed, source) pair for the first time, it should calculate the difference using diff_function 
    and cache that value along with the limit used as a (value, limit) tuple pair.
    If memoized encounters the (typed, source) pair again, it should return the memoized value 
    if the provided limit is less than or equal to the cached limit. 
    Otherwise, the difference should be recalculated, recached, and returned.
    """
    cache ={}
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for index in range(len(word_list)):
        if word_list[index] == typed_word:
            return typed_word
        elif diff_function(typed_word,word_list[index],limit) <= limit:
            dic=dic|{index:diff_function(typed_word,word_list[index],limit)}
    if not dic:
        return typed_word
    object=min([dic[x] for x in dic])
    return word_list[min([index for index in dic if dic[index]==object])]
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    """via recursion  the key is judging the limit in every turn
       in each turn, this function need to judge weather typed's first word eqal source
       the problem is that i need to storage the return
    """
    if limit<0:
        return 14114514
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    numerator=0
    for index in range(min(len(typed),len(source))):
        if typed[index]==source[index]:
            numerator+=1
        else:
            break
    progress=(numerator/len(source))
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[]for _ in timestamps_per_player]  
    for index_player in range(len(timestamps_per_player)):
        for index_word_time in range(len(timestamps_per_player[index_player])):
            if index_word_time<=(len(timestamps_per_player[index_player])-2):
                times[index_player]+=[timestamps_per_player[index_player][index_word_time+1]-timestamps_per_player[index_player][index_word_time]]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        min=get_time(times,0,word_index)
        obj=0
        for player_index in player_indices:
            if player_index <=len(times)-1:
                if get_time(times,player_index,word_index)<min:
                    min=get_time(times,player_index,word_index)
                    obj=player_index
            else:
                continue
        s+=[obj]

...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        self.is_doubled=False  # 实例属性，每只蚂蚁独立
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif(not isinstance(place.ant,ContainerAnt) and isinstance(self,ContainerAnt) and self.can_contain(place.ant)):
    ...
    def double(self):
        ...
        if(not self.is_doubled ):
            ...
            if(isinstance(self,ContainerAnt) and self.ant_contained): # 如果是容器蚂蚁且包含了其他蚂蚁
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        index=0
        aim=self.place
        while(aim and not aim.is_hive): # 一直向后遍历地块
            if(index<=self.upper_bound and aim.bees and index>=self.lower_bound):
                return random_bee(aim.bees)
            aim=aim.entrance
            index+=1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if(self.health-amount<=0):
            ...
            total=amount
        cache = self.place.bees[:]
        for bee in cache:
            bee.reduce_health(total)
...
class TankAnt (ContainerAnt):
    ...
    def action(self, gamestate):
        if(self.place.bees):
            bees=self.place.bees[:] # 复制一份，防止在遍历时修改列表.
            for bee in bees:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while(aim): # 向前遍历,双倍伤害
            if(aim.ant):
                aim.ant.double()
                if(isinstance(aim.ant,ContainerAnt) and aim.ant.ant_contained):
                    aim.ant.ant_contained.double()
            aim=aim.exit
        super().action(gamestate)
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        for __ in self.aimbees:
            if( __ is not target):
                self.aimbees[__]-=1
                if(self.aimbees[__]==0):
                    del(self.aimbees[__])
        if( not self.aimbees[target]):
            self.bees[target]=self.times
        else:
            self.bees[target]+=1
        for __ in self.aimbees:
            if(not self.aimbees[__]%2==0):
                __.action=__.original_action
            else:
                __.action=__.slowed_action
